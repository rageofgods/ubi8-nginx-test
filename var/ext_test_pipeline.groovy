def call(Map pipelineParams) {
    pipeline {
        agent {
            label pipelineParams.deploy_env
        }
        environment {
            DOCKERFILE_NAME = pipelineParams.dockerfile_name
            TEMPLATE_NAME = pipelineParams.template_name
            DOCKER_REGISTRY = pipelineParams.docker_registry
            DOCKER_REGISTRY_AUTH_ID = pipelineParams.docker_registry_auth_id
            BUILD_NAME = pipelineParams.build_name
            RH_REGISTRY = pipelineParams.rh_registry
            RH_REGISTRY_AUTH_ID = pipelineParams.rh_registry_auth_id
            OCP_URL = pipelineParams.ocp_url
            OCP_AUTH_ID = pipelineParams.ocp_auth_id
            IMAGESTREAM = pipelineParams.imagestream
            GIT_COMMIT_SHORT = sh(
